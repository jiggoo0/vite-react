...
const Home: FC = () => {
  ...
  const renderSuspense = (component: ReactNode, fallbackMessage: string) => (
    <Suspense fallback={<div className="text-center py-16 animate-pulse">{fallbackMessage}</div>}>
      {component}
    </Suspense>
  );
  ...
  return (
    <main className="flex flex-col scroll-smooth bg-base-200 text-base-content min-h-screen">
      {sections.map(({ id, title, bg, content }) => (
        <PageSection key={id} id={id} title={title} bgClass={bg}>
          {content}
        </PageSection>
      ))}
    </main>
