...
const MetricCard: FC<MetricCardProps> = ({ label, value, icon, className }) => {
  ...
  return (
    ...
    >
      {/* Icon */}
      {icon && (
        <div className="text-primary dark:text-primary-light flex-shrink-0 text-2xl">{icon}</div>
      )}

      {/* Label + Value */}
      <div className="flex flex-col">
        <span id={labelId} className="text-sm font-medium text-gray-500 dark:text-gray-400">
          {label}
        </span>
