"use client";
import { jsx as _jsx } from "react/jsx-runtime";
import TrustDashboard from "./TrustDashboard";
import { ShieldCheckIcon, CheckBadgeIcon, SparklesIcon } from "@heroicons/react/24/solid";
/** -----------------------------
 * Mock Metrics
 * -----------------------------
 */
const metrics = [
    { key: "exp", label: "ประสบการณ์ทีม", value: "10 ปี" },
    { key: "satisfaction", label: "ความพึงพอใจ", value: "99%" },
    { key: "sla", label: "ส่งทันเวลา", value: "100%" },
    { key: "privacy", label: "ความลับ", value: "กฎข้อแรก" },
];
/** -----------------------------
 * Mock Stats
 * -----------------------------
 */
const stats = [
    { key: "customers", label: "ลูกค้ามั่นใจในเรา", count: 1200 },
    { key: "projects", label: "โปรเจกต์สำเร็จ", count: 350 },
    { key: "partners", label: "พันธมิตร", count: 25 },
];
/** -----------------------------
 * Mock Badges
 * -----------------------------
 */
const badges = [
    {
        id: 1,
        icon: _jsx(ShieldCheckIcon, { className: "h-10 w-10 text-green-500" }),
        title: "Security",
        description: "ข้อมูลปลอดภัยและเชื่อถือได้",
    },
    {
        id: 2,
        icon: _jsx(CheckBadgeIcon, { className: "h-10 w-10 text-blue-500" }),
        title: "Certified Service",
        description: "ได้รับการรับรองมาตรฐานคุณภาพ",
    },
    {
        id: 3,
        icon: _jsx(SparklesIcon, { className: "h-10 w-10 text-yellow-400" }),
        title: "Premium Experience",
        description: "ประสบการณ์ใช้งานราบรื่นและมั่นใจ",
    },
];
/** -----------------------------
 * TrustDashboardDemo Component
 * -----------------------------
 */
const TrustDashboardDemo = () => {
    return _jsx(TrustDashboard, { metrics: metrics, stats: stats, badges: badges });
};
TrustDashboardDemo.displayName = "TrustDashboardDemo";
export default TrustDashboardDemo;
