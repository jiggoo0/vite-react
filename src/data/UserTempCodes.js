...
const userTempCodes = rawUserCodes.map(({ userId, hashedCode }) => ({
    userId,
    hashedCode,
...
export async function resetUserTempCode(userId, plainCode, expiresInMin = 10) {
...
export function getUserTempTimeRemaining(userId) {
    const code = userTempCodes.find((c) => c.userId === userId);
    if (!code || !code.loginStartTime)
        return null;
    const now = Date.now();
